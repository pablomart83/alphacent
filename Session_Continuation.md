...
**Architecture pass 1 (Jun 11 2026, ~11:20 UTC): F31 + A2 + A4 deployed; A1 design doc written. F31 mass-closure guard in `_process_pending_closures` (≤25 closes/cycle, CRITICAL alert, breach-prioritized). A2 staleness unification: new `positions.price_updated_at` (migration `migrate_position_price_updated_at.sql` applied live, 2745 rows backfilled), stamped each successful sync (scoped to positions present on eToro), breach enforcement annotates price age + CRITICAL-escalates stale-price LIVE stops. A4 silent-except detector `scripts/check_silent_excepts.py` (135 in critical dirs; CI via `--ci`, `# silent-ok` to allowlist) + bumped live_trade_count + ghost-position swallows debug→warning. A1 (typed notional) design at `docs/DESIGN_A1_typed_notional.md` — NOT yet implemented (review the migration plan first). NOTE: one transient cold-start error cluster 11:20–11:22 (statement-timeout on first post-restart TSL cycle + 1 DIA demo exit retry) — self-recovered by 11:21:55, did not recur.**

...
- **[P1, PORTFOLIO/CIO] Live book is a single long-tech-momentum factor.** 100% long, 0 shorts, 0 non-tech; SOXL+TQQQ are 3x leveraged. Net-negative ex-PANW. No code bug, but no diversification = correlated drawdown risk on real capital. CIO: cap distinct-correlated-names or require a non-tech/short sleeve before adding live pairs. (The book IS within validate_signal's per-symbol/heat caps — the gap is a *factor/correlation* cap, which validate_signal's correlation-cluster check should be covering on live; verify it actually fires.)
- **[P1, RESEARCH] Graduation PAPER-sample selection bias (real, ongoing).** Demo book saturated (321 PAPER+BACKTESTED strategies vs ~193 position slots; equity $560K but balance only $18K ≈94% deployed). WHICH pairs accumulate the min_trades=15 paper sample is balance-gated (first-come-first-served), so `qualification_ratio` is computed on a biased sample and pairs that never get a paper fill can never graduate regardless of edge. Gate *math* is sound (Wilson LB, type floors, ratio 0.6–2.0). The *sample* is biased. Demo capacity is USER-OWNED, but the statistical consequence for graduation soundness should be on the record. Proper fix = simulated shadow paper book (no balance constraint) for graduation sampling.
- **[P2, LIVE cleanup] Dead/duplicate nested live-fill path** at `trading_scheduler.py:~1655` (inside the DEMO submission loop). Superseded by the independent live pass (`:~2388`, the correct path: per-pair `conviction_min`, dup+concentration guard, validate_signal, CIO sizing). The nested path uses the YAML-default conviction (no per-pair, **no alpha_edge branch**) and can't fire for live pairs anyway (live strategies are status='LIVE', the DEMO loop only iterates PAPER+BACKTESTED at `:325`). Remove it to avoid a future double-fire / inferior-gate footgun. Needs confirmation (code deletion).
- **[P2, PAPER] CAT (and similar) recurring `Order size must be at least $1000` ERROR every cycle.** PAPER strategy; flat $1000 paper size gets shaved below $1000 by the ATR-SL-widening size rescale (preserve-dollar-risk) → fails the $1000 demo floor → never paper-trades + logs at ERROR (noise in errors.log). PAPER data-collection gap + observability. Proper fix: in PAPER, floor the post-rescale size up to `min_order_size` (data collection > dollar-risk precision in the sandbox), and log expected sub-floor skips at INFO not ERROR.
- **[P2, COSTS — bundled, data-gated] F1/F2 (gating Sharpe/WR gross of costs) + F3 (1d Sharpe calendar-annualized ×1.204).** Unchanged. Prereq Fix D: derive empirical per-asset costs from the now-clean slippage data (needs a few market-hours days) + repair contaminated historical `entry_slippage`. Then route costs into `from_signals(fees=…)`, correct the 1d annualization, and re-baseline WF/conviction/graduation thresholds in ONE pass.
...
- **Verify next market-hours cycle:** retire→PAPER revert fires on the next live retirement (`status_reverted_to_paper:true` in the response + `[status LIVE→PAPER]` log); demo 604/CAT noise; no new `uq_open_pos`.

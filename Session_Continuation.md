...
- **[NEXT — recommended] WF-Sharpe robustness across version churn.** The graduation fail-closed (missing WF) can TRANSIENTLY block strong-paper pairs (e.g. XLK/MU were briefly shown "wf_sharpe unavailable") when a template's WF-carrying strategy versions are deleted by the TTL and the re-proposed versions haven't re-validated yet. Self-heals per cycle, but the "edge unestablished" message is misleading for pairs whose WF edge WAS established. **Proper fix: persist/recover the WF Sharpe at the (template, symbol) or template level so it survives version deletion** (same class as the trade-history loss already fixed; NOT entangled with the cost work). Contained, ~half day.

...
- **[GRADUATION-LOGIC review 2026-06-18] (a) Dead WF-Sharpe-CI gate — REMOVED (commit `5c759b2`).** Depended on `wf_test_trades` (0/328 populated) → never fired; redundant with + statistically weaker than the upstream MC bootstrap (p5 Sharpe≥0, distribution-free) and WF min-test-trade guards a pair already clears. Removed rather than wired (no dead/weaker safety theater on a real-money path). Optional follow-up: surface `wf_test_trades` as CIO-card metadata (flag-don't-block if thin). **(b) STILL OPEN — qualification_ratio MAX cap is structurally biased**: paper Sharpe (flat $1K, √252 on 15–28 trades, noisy/inflated) vs WF Sharpe (vol-scaled) — not apples-to-apples; over-rejects eligible pairs (blocks SPY at 4.44×>3.5). Proper fix = compute paper Sharpe consistently with (and net-of-cost vs) WF — bundle with F1/F2, don't patch piecemeal. **(c) Minor:** alpha-gate benchmark SQL uses `(MAX−MIN)/MIN` range not period return; low priority. ($5K-notional bug already fixed `48b3d58`.)

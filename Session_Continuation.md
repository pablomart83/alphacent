...
**#6 COSTS (F1/F2/F3) — NOT shipped (correctly): no proper fix ready, data-gated.** F3-in-isolation is forbidden; the bundled cost-Sharpe recalibration is gated on **Fix D** (empirical per-asset costs). Fix D is **doubly blocked, evidenced this session**:
  ...
  2. **exit_slippage is still 100% NULL on all 127 closes since 06-14** (125 demo + 2 live) — the 06-14 "exit slippage now captured" fix (`a69cb6b`) is **NOT actually capturing**. Entry slippage populates; exit does not. A `log_exit` caller on the close path is not passing the new `expected_exit_price`/`exit_submitted_at`/`exit_order_side` args (or the drift guard nulls them all). **This is the concrete unblocker for the whole cost recalibration — fix/verify exit-slippage capture FIRST.**
  Proper sequence (design, confirm): (a) repair exit-slippage capture + verify on real closes; (b) accumulate clean LIVE fills over several market days; (c) derive empirical per-asset costs + repair contaminated historical `entry_slippage`; (d) THEN route costs into `from_signals(fees=…)` + add the 1d annualization branch (×√(252/365) equity/index/commodity, √(260/365) forex, none crypto) + re-baseline WF/conviction/graduation in ONE pass.

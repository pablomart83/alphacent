...
- **[P2, GRADUATION-LOGIC review 2026-06-18] Two gate issues found (beyond trade count).** (a) **Dead WF-Sharpe-CI gate**: `is_qualified` improvement-3 (reject if WF Sharpe lower-CI ≤ 0) is gated on `wf_test_trades`, which is populated in **0/328 strategies** → the gate has never fired. Either wire `wf_test_trades` at WF time or remove it (redundant with MC-bootstrap p5-Sharpe). (b) **qualification_ratio MAX cap is structurally biased**: paper Sharpe (flat $1K, √252 on 15–28 trades, noisy/inflated) vs WF Sharpe (vol-scaled) — not apples-to-apples; the cap over-rejects eligible pairs (it's what blocks the lone new candidate SPY at 4.44×>3.5). Proper fix = compute paper Sharpe consistently with (and net-of-cost vs) WF — bundle with the F1/F2 cost-Sharpe recalibration, don't patch piecemeal. (c) Minor: alpha-gate benchmark SQL uses `(MAX−MIN)/MIN` peak-to-trough RANGE, not period return (overstates benchmark); low priority. The $5K-notional bug was already fixed (`48b3d58`).

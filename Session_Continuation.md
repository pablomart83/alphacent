...
**#6 COSTS (F1/F2/F3) — NOT shipped (correctly): no proper fix ready, data-gated.** F3-in-isolation is forbidden; the bundled cost-Sharpe recalibration is gated on **Fix D** (empirical per-asset costs). Fix D is **doubly blocked, evidenced this session**:
  ...
  2. **exit_slippage is still 100% NULL on all 127 closes since 06-14** (125 demo + 2 live). **ROOT-CAUSED this session:** the ONLY close path that captures exit slippage is `order_executor._handle_buy_fill/_handle_sell_fill` (correctly passes `expected_exit_price=order.expected_price`, `exit_price=fill.filled_price`, side, submit time — the 06-14 fix is correct *there*). But that fill-handler path is **dead for production closes** — every real close (TSL breach + zombie + retirement + partial via `monitoring_service:~4004`, the canonical `position_close.finalize_position_close:104`, and eToro-side SL/TP via `order_monitor` sync `:477/:2357`) calls `log_exit` with only `trade_id/exit_time/exit_price/exit_reason/symbol`, uses `pos.current_price` (last sync) as the exit price, and **never captures eToro's actual executed close rate**. So even adding the args would compare current_price vs current_price = a dishonest ~0. eToro's real close rate is only available via `etoro_client.get_trade_history(min_date)` (LIVE-only; demo fills are at-quote ≈ 0 slippage). **Honest fix is structural + LIVE-data-sparse — designed, NOT shipped (a naive in-hot-path wiring would fabricate 0-slippage, violating no-dishonest-measurement). Design + confirm before building.**
  Proper sequence (design, confirm): (a) repair exit-slippage capture via an off-hot-path backfill from `get_trade_history` (eToro's real executed close rate) — read-only, no risk to the close path; (b) accumulate clean LIVE fills over several market days; (c) derive empirical per-asset costs + repair contaminated historical `entry_slippage`; (d) THEN route costs into `from_signals(fees=…)` + add the 1d annualization branch (×√(252/365) equity/index/commodity, √(260/365) forex, none crypto) + re-baseline WF/conviction/graduation in ONE pass.

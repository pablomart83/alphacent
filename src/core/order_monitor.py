        ...
        try:
            ...
            if len(positions) == 0 and len(open_db_positions) > 3:
                ...
                for db_pos in open_db_positions:
                    ...
                    # Log to trade journal — eToro closed this position (SL/TP hit or manual).
                    # Create entry + exit if no entry exists (positions from sync never went through OrderExecutor).
                    try:
                        from src.analytics.trade_journal import TradeJournal
                        journal = TradeJournal(self.db)
                        ...
                        # Store exit reason on position for the closed positions UI

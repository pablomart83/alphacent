...
| Crypto (Diamond, $100K-$250K) | **0.3% per position** | $0 | $0 (non-leveraged) | CFD: varies |
...
**Critical:** Non-leveraged stock/ETF BUY positions have **zero overnight fee and zero spread markup**. The only real cost is market impact (slippage ~2 bps). Crypto round-trip is **0.6% commission (0.3% open + 0.3% close)**, ~0.7–0.8% incl. slippage. (Account-holder-confirmed Diamond crypto rate, 2026-06-20: **0.3%/side**, NOT the 0.75%/side previously modelled. eToro's *public* crypto rate is 1%/side; the Diamond/account rate is 0.3%/side — trust the account holder.) These are reflected in `config/autonomous_trading.yaml` `backtest.transaction_costs`.

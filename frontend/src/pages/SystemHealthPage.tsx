          ...
          {(hasOpenCB || hasStaleMonitoring) && (
            <div className="rounded-lg border p-3" style={{ backgroundColor: 'rgba(239, 68, 68, 0.1)', borderColor: 'rgba(239, 68, 68, 0.3)' }}>
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[11px] font-semibold text-red-400">System Alert</p>
            ...
            <>
              ...
              <div className="rounded-lg border" style={{ backgroundColor: 'var(--color-dark-surface)', borderColor: 'var(--color-dark-border)', minHeight: '200px', maxHeight: '40%' }}>
                <div className="px-3 py-2 border-b" style={{ borderColor: 'var(--color-dark-border)' }}>
                  <span className="text-xs text-gray-500 tracking-wide font-medium">24-Hour Event Timeline</span>
                </div>
                {(data.events_24h ?? []).length > 0 ? (
                      ...
                      <div key={i} className="flex items-start gap-2 py-1" style={{ borderBottom: '1px solid var(--color-dark-border)' }}>
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[11px] font-mono px-1 py-0.5 rounded" style={{ backgroundColor: 'var(--color-dark-bg)', color: 'var(--color-text-secondary)' }}>
                              {evt.type}
                            </span>
                          </div>
                          <p className="text-[11px] mt-0.5 truncate" style={{ color: 'var(--color-text-primary)' }}>{evt.description}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center justify-center p-6">
                    <p className="text-[11px]" style={{ color: 'var(--color-text-secondary)' }}>No events in the last 24 hours.</p>
                  </div>
                )}
              </div>
              ...
              <div className="rounded-lg border p-3" style={{ backgroundColor: 'var(--color-dark-surface)', borderColor: 'var(--color-dark-border)' }}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs text-gray-500 tracking-wide font-medium">Monitoring Service</span>
                  <span className={`text-[11px] font-mono px-1.5 py-0.5 rounded ${data.monitoring_service?.running ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                  ...
                  {(data.monitoring_service?.sub_tasks ?? []).map((task) => (
                    <div key={task.name} className="rounded-lg p-2" style={{ backgroundColor: 'var(--color-dark-bg)' }}>
                      <div className="flex items-center gap-1.5 mb-1">
                        ...
                        <p className="text-[11px] font-mono truncate" style={{ color: 'var(--color-text-secondary)' }}>{task.name}</p>
                      </div>
                      <p className="text-[11px] font-mono" style={{ color: 'var(--color-text-primary)' }}>{formatAge(task.last_cycle)}</p>
              ...
              {/* Trading Scheduler */}
              <div className="rounded-lg border p-3" style={{ backgroundColor: 'var(--color-dark-surface)', borderColor: 'var(--color-dark-border)' }}>
                <span className="text-xs text-gray-500 tracking-wide font-medium">Trading Scheduler</span>
                  ...
                  {['quick_price_update', 'full_price_sync'].map((key) => {
                    ...
                    return (
                      <div key={key} className="rounded-lg p-2" style={{ backgroundColor: 'var(--color-dark-bg)' }}>
                        <p className="text-[11px] font-mono mb-1" style={{ color: 'var(--color-text-primary)' }}>
      ...
      <PanelHeader title="Alerts" panelId="syshealth-side" onRefresh={fetch}>
        <div className="flex flex-col gap-2 p-2 h-full">
          ...
          <div className="border border-[var(--color-dark-border)] rounded-lg p-3">
            <div className="text-xs text-gray-500 tracking-wide font-medium mb-2">Circuit Breakers</div>
            <div className="grid grid-cols-1 gap-2">
              {(data?.circuit_breakers ?? []).map((cb) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="text-xs font-mono font-semibold capitalize" style={{ color: 'var(--color-text-primary)' }}>{cb.category}</span>
                  ...
                  <div className="flex items-center gap-3">
                    <div>
                      ...
                      <p className="text-xs font-mono" style={{ color: cb.failure_count > 0 ? '#ef4444' : 'var(--color-text-primary)' }}>
                        {cb.failure_count}
                      </p>
                    </div>
                    {cb.cooldown_remaining_seconds > 0 && (
                      <div>
                        ...
                        <p className="text-xs font-mono text-amber-400">{cb.cooldown_remaining_seconds}s</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {(data?.circuit_breakers ?? []).length === 0 && (
                <p className="text-[11px]" style={{ color: 'var(--color-text-secondary)' }}>No circuit breaker data available.</p>
              )}
            </div>
          </div>
          ...
          {data?.etoro_api && (
            <div className="border border-[var(--color-dark-border)] rounded-lg p-3">
              <div className="text-xs text-gray-500 tracking-wide font-medium mb-2">eToro API Health</div>
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-lg p-2" style={{ backgroundColor: 'var(--color-dark-bg)' }}>
                  ...
                  <p className="text-xs font-mono" style={{ color: 'var(--color-text-primary)' }}>{data.etoro_api.requests_per_minute ?? '—'}</p>
                </div>
                <div className="rounded-lg p-2" style={{ backgroundColor: 'var(--color-dark-bg)' }}>
                  <p className="text-[8px]" style={{ color: 'var(--color-text-secondary)' }}>Error Rate (5m)</p>
                  <p className="text-xs font-mono" style={{ color: (data.etoro_api.error_rate_5m ?? 0) > 5 ? '#ef4444' : '#22c55e' }}>
                ...
                <div className="rounded-lg p-2" style={{ backgroundColor: 'var(--color-dark-bg)' }}>
                  ...
                  <p className="text-xs font-mono" style={{ color: (data.etoro_api.avg_response_ms ?? 0) > 2000 ? '#eab308' : 'var(--color-text-primary)' }}>
                ...
                <div className="rounded-lg p-2" style={{ backgroundColor: 'var(--color-dark-bg)' }}>
                  ...
                  <p className="text-xs font-mono" style={{ color: (data.etoro_api.rate_limit_remaining ?? 100) < 20 ? '#ef4444' : 'var(--color-text-primary)' }}>

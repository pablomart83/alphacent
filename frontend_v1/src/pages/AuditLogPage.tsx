        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-2 border-b" style={{ borderColor: 'var(--color-dark-border)', backgroundColor: 'var(--color-dark-surface)' }}>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <select
                ...
                className="text-xs font-mono rounded px-1.5 py-1 border"
                style={{ backgroundColor: 'var(--color-dark-bg)', borderColor: 'var(--color-dark-border)', color: 'var(--color-text-primary)' }}
              >
              ...
              <input
                ...
                placeholder="Symbol"
                className="w-20 px-1.5 py-1 rounded text-xs font-mono border"
                style={{ backgroundColor: 'var(--color-dark-bg)', borderColor: 'var(--color-dark-border)', color: 'var(--color-text-primary)' }}
              />
              <input
                type="text"
                ...
                className="w-24 px-1.5 py-1 rounded text-xs font-mono border"
                style={{ backgroundColor: 'var(--color-dark-bg)', borderColor: 'var(--color-dark-border)', color: 'var(--color-text-primary)' }}
              />
              <input
                type="date"
                ...
                className="px-1.5 py-1 rounded text-xs font-mono border"
                style={{ backgroundColor: 'var(--color-dark-bg)', borderColor: 'var(--color-dark-border)', color: 'var(--color-text-primary)' }}
              />
          ...
          {entries.length > 0 ? (
            <div className="flex-1 min-h-0 flex flex-col">
              {/* Table Header */}
              <div className="flex items-center text-xs font-mono py-1.5 px-3 border-b" style={{ borderColor: 'var(--color-dark-border)', color: 'var(--color-text-secondary)', backgroundColor: 'var(--color-dark-bg)' }}>
                  ...
                  {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <span className="w-[140px] shrink-0 font-mono text-xs" style={{ color: 'var(--color-text-secondary)' }}>
                            {new Date(entry.timestamp).toLocaleString()}
                          </span>
                          <span className="w-[120px] shrink-0 text-xs font-mono" style={{ color: 'var(--color-text-primary)' }}>
                            {entry.event_type?.replace(/_/g, ' ')}
                          </span>
                          <span className="w-[70px] shrink-0 font-mono text-xs" style={{ color: 'var(--color-text-primary)' }}>
                            {entry.symbol || '—'}
                          </span>
                          <span className="w-[120px] shrink-0 text-xs truncate" style={{ color: 'var(--color-text-secondary)' }}>
                            {entry.strategy_name || '—'}
                          </span>
              ...
              {loading && <div className="text-center py-1 text-xs font-mono" style={{ color: 'var(--color-text-secondary)' }}>Loading more...</div>}
              <div className="px-3 py-1 border-t text-xs font-mono" style={{ borderColor: 'var(--color-dark-border)', color: 'var(--color-text-secondary)' }}>
        ...
        <div className="flex flex-col gap-1 p-1.5 h-full">
          ...
          <div className="border border-[var(--color-dark-border)] rounded-lg">
            <div className="px-3 py-1.5 border-b" style={{ borderColor: 'var(--color-dark-border)', backgroundColor: 'var(--color-dark-bg)' }}>
              ...
              <span className="ml-2 text-xs font-mono" style={{ color: '#eab308' }}>({rejections.length})</span>
            </div>
            {rejections.length > 0 ? (
                ...
                {rejections.slice(0, 10).map((e) => (
                  <div key={e.id} className="px-3 py-1.5" style={{ borderBottom: '1px solid var(--color-dark-border)' }}>
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-mono" style={{ color: 'var(--color-text-primary)' }}>{e.symbol || '—'}</span>
                      <span className="text-xs font-mono" style={{ color: 'var(--color-text-secondary)' }}>
                        {new Date(e.timestamp).toLocaleTimeString()}
                      </span>
                    </div>
                    <p className="text-xs mt-0.5 truncate" style={{ color: '#eab308' }}>
              ...
              <div className="p-3 text-center">
                <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>No signal rejections.</p>
              </div>
            )}
          </div>
          ...
          <div className="border border-[var(--color-dark-border)] rounded-lg">
            <div className="px-3 py-1.5 border-b" style={{ borderColor: 'var(--color-dark-border)', backgroundColor: 'var(--color-dark-bg)' }}>
              ...
              <span className="ml-2 text-xs font-mono" style={{ color: 'var(--color-text-secondary)' }}>({lifecycleEvents.length})</span>
            </div>
            {lifecycleEvents.length > 0 ? (
              <div className="overflow-y-auto" style={{ maxHeight: '180px' }}>
                {lifecycleEvents.slice(0, 10).map((e) => (
                  <div key={e.id} className="px-3 py-1.5" style={{ borderBottom: '1px solid var(--color-dark-border)' }}>
                    ...
                    <p className="text-xs mt-0.5 truncate" style={{ color: 'var(--color-text-primary)' }}>
                      {e.strategy_name || '—'}
                    </p>
                  </div>
                ))}
              ...
              <div className="p-3 text-center">
                <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>No lifecycle events.</p>
              </div>
            )}
          </div>
          ...
          <div className="border border-[var(--color-dark-border)] rounded-lg">
            <div className="px-3 py-1.5 border-b" style={{ borderColor: 'var(--color-dark-border)', backgroundColor: 'var(--color-dark-bg)' }}>
              ...
              <span className="ml-2 text-xs font-mono" style={{ color: 'var(--color-text-secondary)' }}>({riskEvents.length})</span>
            </div>
            {riskEvents.length > 0 ? (
              <div className="overflow-y-auto" style={{ maxHeight: '180px' }}>
              ...
              <div className="p-3 text-center">
                <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>No risk limit events.</p>
              </div>
            )}
          </div>
        </div>
...
const TradeLifecycleView: FC<{ data: TradeLifecycleData | null; loading: boolean; entry: AuditLogEntry }> = ({ data, loading, entry }) => {
  ...
  if (!data) {
    return (
      <div className="px-6 py-3" style={{ backgroundColor: 'var(--color-dark-bg)' }}>
        ...
        {entry.metadata && (
          <pre className="text-xs font-mono mt-1.5 p-2 rounded overflow-x-auto" style={{ backgroundColor: 'var(--color-dark-surface)', color: 'var(--color-text-secondary)' }}>
  ...
  return (
    <div className="px-6 py-3" style={{ backgroundColor: 'var(--color-dark-bg)' }}>
      <p className="text-xs font-mono mb-2" style={{ color: 'var(--color-text-primary)' }}>Trade Lifecycle</p>
